...
function convertJikanAnimeToAnime(jikanAnime: JikanAnime): Anime {
  return {
    ...
    rating: jikanAnime.score ? `${jikanAnime.score}/10` : '',
...
function convertJikanMangaToManga(jikanManga: JikanManga): Manga {
  return {
    ...
    releaseDate: jikanManga.published?.from || '',
    author: jikanManga.authors?.[0]?.name || 'Unknown',
    rating: jikanManga.score ? `${jikanManga.score}/10` : '',
